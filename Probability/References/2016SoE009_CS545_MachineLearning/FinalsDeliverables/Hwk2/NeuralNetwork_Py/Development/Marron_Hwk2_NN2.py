...
class NN(object):
    def __init__(self, NNsize):
        ...
        nabla_btp = [np.zeros(b.shape) for b in self.biases]
        nabla_wtp = [np.zeros(w.shape) for w in self.weights]
    ...
    def SGD(self, tr_d, epochs, eta, alpha, te_d=None):
        ...
        for j in xrange(epochs):
            for datum in tr_d:
                self.update_perdatum(datum, eta, alpha)
            if te_d:
    ...
    def update_perdatum(self, datum, eta, alpha):
        ...
        self.weights = [w+((eta)*nw+(alpha)*ow) for w, nw, ow in zip(self.weights, nabla_w, nabla_wtp)]
        self.biases = [b+((eta)*nb+(alpha)*ob) for b, nb, ob in zip(self.biases, nabla_b, nabla_btp)]
        nabla_btp=nabla_b
        nabla_wtp=nabla_w
