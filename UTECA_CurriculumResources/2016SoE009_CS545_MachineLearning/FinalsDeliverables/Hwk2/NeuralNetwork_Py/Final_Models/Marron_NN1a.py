"""
Title:			A Neural Network for Letter Recognition
Project Descriptor:	Hwk #2: Letter Recognition with a 3-Layer NN
...
from development model: Marron_Hwk2_NN4.py
@author: bmarron
"""
...
class LtrRecogNN(object):
    def __init__(self, NNsize):
		...
		#forward propagation for the evaluate method
    def feedforward(self, a):
		...
		#stochastic gradient descent method that calls the update_perdatum method    
    def SGD(self, tr_d, epochs, eta, alpha, te_d=None):
        if te_d:
            n_te_d = len(te_d)        
        for j in xrange(epochs):
            for k in xrange(len(tr_d)):
                self.update_perdatum(tr_d[k], eta, alpha)
            if te_d:
		...
		# updating the biases and weights after calling the backpropagation method
    def update_perdatum(self, datum, eta, alpha):
        x=datum[0]
        y=datum[1]
        nabla_b, nabla_w = self.backprop(x, y)        
        self.weights = [w+((eta)*nw+(alpha)*ow) for w, nw, ow in zip(self.weights, nabla_w, self.nabla_wtp)]
        self.biases = [b+((eta)*nb+(alpha)*ob) for b, nb, ob in zip(self.biases, nabla_b, self.nabla_btp)]
    ...
    def backprop(self, x, y):
        ...
        #the error (function) method
    def error(self, a_L3, y): 
        return (a_L3-y)  
        
        #the evaluate method
	#compares the max values in the ouput vector and the target vector and returns a "1" if TRUE, "0" otherwise
	#sums the number of "1s"
